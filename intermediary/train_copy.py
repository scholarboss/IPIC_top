...
elif args.model == '6': # res
    model = DnText_res(num_class=1024+args.pilot*4).cuda()
...
data1.seek(4*2*2*2*32*(300000-1))
# H1=struct.unpack('f'*2*2*2*32*320000,data1.read(4*2*2*2*32*320000))
# H1=np.reshape(H1,[320000,2,4,32])
# H=H1[:,1,:,:]+1j*H1[:,0,:,:]

H1=struct.unpack('f'*2*2*2*32*(20000+1),data1.read(4*2*2*2*32*(20000+1)))
H1=np.reshape(H1,[20000+1,2,4,32])
H=H1[:,1,:,:]+1j*H1[:,0,:,:]
Htest=H[1:,:,:]
H=H[:1,:,:]
...
train_loader = torch.utils.data.DataLoader(train_data, batch_size=args.batchsize, shuffle=True, num_workers=6)
test_data = ChannelDataLoader(Htest, pilot=args.pilot)
test_loader = torch.utils.data.DataLoader(test_data, batch_size=args.batchsize, shuffle=False, num_workers=2)
...
for epoch in range(args.start, args.end):
    ...
    with torch.no_grad():
        for i, (input_, label_) in enumerate(test_loader):
            ...
            print('batch num: {} error: {} score: {:6f}'.format(i, error, score))
            total_score += score
        ...
        #     modelSave = './Modelsave/model_pilot{}_tag{}.pth'.format(args.pilot, args.tag)
        #     state = {
